...
class PanelPage extends Component {
    ...
    constructor(props) {
        ...
        this.itemColor = React.createRef();
    ...
    createPost = () => {
        ...
        axios.post("http://localhost:3002/products", {
            ...
            "color":this.itemColor.current.value,
            "createdAt": "",
    ...
    setEditId = (id) => {
        ...
        setTimeout(this.editData, 200);
        this.openModal();
    }
    editData = () => {
        const axios = require('axios');
        axios.get(`http://localhost:3002/products/${this.state.editId}`)
            .then(res => {
                const editData = res.data;
                this.itemName.current.value = editData.firstName;
                this.itemBrand.current.value = editData.brand;
                this.itemPrice.current.value = editData.price;
                this.itemCount.current.value = editData.count;
                this.itemColor.current.value = editData.count;
                this.itemCate.current.value = editData.category;

                // axios.get(`http://localhost:3002/files/${editData.image}`)
                // .then(res => {
                //     this.itemImg.current.value.files[0] = res.data;
                // })
            })
    }
    setDelID = (id) => {
                    ...
                    <div className="relative">
                        ...
                        {this.state.modal &&
                            <div className="flex justify-center items-center modal-bg">
                                <form className="dir-r w-2/3 bg-white px-8 py-8 rounded-lg form-post">
                                    ...
                                    <label>
                                        دسته بندی :
                                        <select className="hover:cursor-pointer" ref={this.itemCate}>
                                    ...
                                    <div onClick={this.handlePost} className="bg-green-500 py-2 flex items-center hover:cursor-pointer justify-center  rounded-lg">ثبت</div>
                                </form>
                            </div>
                        }
                        ...
                        {this.state.activePage == "items" &&
                            <div className="flex mt-20 mb-1 w-full flex-col items-center mb-8">
                                <button onClick={this.openModal} id="createNew" className="bg-green-400 py-2 px-4 hover:cursor-pointer text-xl rounded-lg">ثبت کالای جدید</button>
