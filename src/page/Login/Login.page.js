...
function LoginPage() {
  ...
  const Login = () => {
    ...
    const result = users.find((item) => item.username === user.email && item.password === user.password);
    if (result) {
      navigator(`/PanelPage`);
    } else {
    //   alert("لطفا فیلد هارا به درستی پر کنید");
    setError('لطفا فیلد هارا به درستی کامل کنید')
    }
